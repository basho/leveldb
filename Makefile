...
OPT ?= -std=c++0x -O3 -g -DNDEBUG    # (A) Production use (optimized mode)
# OPT ?= -std=c++0x -g2             # (B) Debug mode, w/ full line-level debugging symbols
# OPT ?= -std=c++0x -O3 -g2 -DNDEBUG # (C) Profiling mode: opt, but w/debugging symbols
#-----------------------------------------------
...
COPT:= $(subst -std=c++0x,,$(OPT))

CFLAGS += -I. -I./include $(PLATFORM_CCFLAGS) $(COPT)
